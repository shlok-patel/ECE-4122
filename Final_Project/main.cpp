...
double ypos = 24.5, xpos = 1, zpos = 1.4;
...
void handleKeypress(unsigned char key, int xx, int yy) {
    if (key == 's')    // r key rotates board 10 degrees counterclockwise
    {
        // increment angle
        ypos-=0.5;
        std::cout << "Y: " << ypos << ", X: " << xpos << std::endl;
    }
    if (key == 'w')    // r key rotates board 10 degrees counterclockwise
    {
        // increment angle
        ypos+=0.5;
        std::cout << "Y: " << ypos << ", X: " << xpos << std::endl;
    }
    if (key == 'a')    // r key rotates board 10 degrees counterclockwise
    {
        // increment angle
        xpos-=0.5;
        std::cout << "Y: " << ypos << ", X: " << xpos << std::endl;
    }
    if (key == 'd')    // r key rotates board 10 degrees counterclockwise
    {
        // increment angle
        xpos+=0.5;
        std::cout << "Y: " << ypos << ", X: " << xpos << std::endl;
    }
    if (key == 'r')    // r key rotates board 10 degrees counterclockwise
    {
        // increment angle
        zpos+=0.1;
        std::cout << "Z: " << zpos << ", X: " << xpos << std::endl;
    }
    if (key == 'f')    // r key rotates board 10 degrees counterclockwise
    {
        // increment angle
        zpos-=0.1;
        std::cout << "Z: " << zpos << ", X: " << xpos << std::endl;
    }
    if (key == 'q' || key == 'Q')    // r key rotates board 10 degrees counterclockwise
    {
        // increment angle
        exit(0);
    }
...
void renderScene()
{
    ...
    glLoadIdentity();
//    gluLookAt(yardsToMeters(60.0), 0, xpos,
//              yardsToMeters(60.0), yardsToMeters(54.0/2), 0,
//              0.0, 0.0, 1.0);
...
//        }
//
//        double x = rcvbuffer[i*numElements];
//        double y = rcvbuffer[i*numElements+1];
//        double z = rcvbuffer[i*numElements+2];
//
//        double dist = sqrt(pow(sendBuffer[0] - x, 2) + pow(sendBuffer[1] - y, 2) + pow(sendBuffer[2] - z, 2));
//
//        if(dist <= 0.5 && i != lastCollision)
//        {
//            sendBuffer[3] = rcvbuffer[i*numElements + 3];
//            sendBuffer[4] = rcvbuffer[i*numElements + 4];
//            sendBuffer[5] = rcvbuffer[i*numElements + 5];
...
//    if(rank == 0)
//    {
//        MPI_Bcast(&elapsedTime, sizeof(elapsedTime), MPI_DOUBLE, 0, MPI_COMM_WORLD);
//    }
//
//    // receiving broadcast for ranks 1-16
//    if(rank != 0)
//    {
//        MPI_Bcast(&elapsedTime, sizeof(elapsedTime), MPI_DOUBLE, 0, MPI_COMM_WORLD);
//    }
    ...
    else
    {
        ...
        {
            //glutTimerFunc(100, incrementTime, 0);
